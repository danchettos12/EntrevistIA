    ...
    <div className="max-w-4xl mx-auto space-y-8 py-10 animate-fadeIn">
      ...
      <div className="relative">
        <textarea 
          ...
          placeholder="Escriba o grabe su respuesta..."
        />
        ...
        <div className="absolute bottom-6 right-6 flex items-center gap-4">
          {transcribing && (
            <div className="flex items-center gap-2 px-4 py-2 bg-blue-500/5 border border-blue-500/20 rounded-full animate-pulse">
               <span className="text-[9px] font-bold text-blue-400 uppercase tracking-widest">Transcribiendo...</span>
            </div>
          )}
          <button 
      ...
      <div className="flex gap-4 justify-end">
        <button onClick={onCancel} className="px-6 py-4 rounded-xl text-slate-500 font-bold uppercase text-[10px] tracking-widest hover:text-white">Cancelar</button>
        <button 
          onClick={handleNext} 
        ...
        >
          {currentIdx + 1 === config.questionCount ? 'Finalizar Sesión' : 'Siguiente Pregunta'}
        </button>
      </div>
    </div>
  );
