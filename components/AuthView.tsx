  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        if (error) throw error;
        setError('¡Cuenta creada! Revisa tu correo electrónico para confirmar la cuenta o intenta iniciar sesión.');
      }
    } catch (err: any) {
        ...
        <div className="glass p-10 rounded-[2rem] shadow-2xl border-white/10 relative overflow-hidden">
          <div className="text-center mb-10">
            ...
            <h1 className="text-2xl font-bold text-white uppercase tracking-tight">
              {isLogin ? 'Acceso Profesional' : 'Registro de Usuario'}
            </h1>
            <p className="text-slate-400 text-xs mt-2 font-medium uppercase tracking-widest">
              {isLogin ? 'Inicie sesión en su panel' : 'Inicie su entrenamiento hoy'}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {!isLogin && (
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-5 py-4 text-white outline-none focus:border-blue-500 transition-all" 
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest pl-2">Correo Corporativo</label>
              <input 
                type="email" 
                value={email} 
            ...
            {error && (
              <div className={`p-4 rounded-xl text-[10px] font-bold text-center border leading-relaxed ${error.includes('Cuenta creada') ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-red-500/10 border-red-500/20 text-red-400'}`}>
                {error}
              </div>
            )}
            ...
            >
              {loading ? 'Procesando Solicitud...' : isLogin ? 'Ingresar al Sistema' : 'Crear Cuenta Profesional'}
            </button>
          </form>
